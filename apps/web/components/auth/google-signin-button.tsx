'use client';

import { useGoogleLogin } from '@react-oauth/google';
import { useRouter } from 'next/navigation';
import { useState, useCallback } from 'react';
import { Loader2 } from 'lucide-react';
import { toast } from 'sonner';
import { useAuthStore } from '@/lib/stores/auth.store';
import { cn } from '@/lib/utils';

interface GoogleSignInButtonProps {
    className?: string;
    mode?: 'signin' | 'signup';
    onSuccess?: () => void;
    onError?: (error: string) => void;
}

// Google icon SVG
const GoogleIcon = () => (
    <svg className="h-5 w-5" viewBox="0 0 24 24">
        <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        />
        <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        />
        <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        />
        <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        />
    </svg>
);

// Check if Google OAuth is configured
const isGoogleOAuthConfigured = () => {
    return !!process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
};

/**
 * Inner component that uses the Google OAuth hook
 * Only rendered when GoogleOAuthProvider is available
 */
function GoogleSignInButtonInner({
    className,
    mode = 'signin',
    onSuccess,
    onError,
}: GoogleSignInButtonProps) {
    const router = useRouter();
    const { setUser, setAccessToken } = useAuthStore();
    const [isLoading, setIsLoading] = useState(false);

    const handleGoogleAuth = useCallback(async (credential: string) => {
        try {
            setIsLoading(true);

            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
            const response = await fetch(`${apiUrl}/api/v1/auth/google`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ idToken: credential }),
                credentials: 'include',
            });

            const data = await response.json();

            if (!response.ok) {
                throw new Error(data.message || 'Authentication failed');
            }

            // Store auth state
            setUser(data.user);
            setAccessToken(data.tokens.accessToken);

            // Store refresh token in localStorage as backup
            if (data.tokens.refreshToken) {
                localStorage.setItem('refreshToken', data.tokens.refreshToken);
            }

            // Success feedback
            if (data.isFirstTime) {
                toast.success('Account created! Welcome to Free2Talk!');
                // Redirect to complete profile for new users
                router.push('/profile?setup=true');
            } else {
                toast.success('Welcome back!');
                router.push('/');
            }

            onSuccess?.();
        } catch (error) {
            console.error('Google auth error:', error);
            const message = error instanceof Error ? error.message : 'Sign-in failed';
            toast.error(message);
            onError?.(message);
        } finally {
            setIsLoading(false);
        }
    }, [router, setUser, setAccessToken, onSuccess, onError]);

    const googleLogin = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            // For implicit flow, we get access_token
            // We need to exchange it for user info or use credential response
            try {
                setIsLoading(true);

                // Get user info from Google
                const userInfoResponse = await fetch(
                    'https://www.googleapis.com/oauth2/v3/userinfo',
                    {
                        headers: {
                            Authorization: `Bearer ${tokenResponse.access_token}`,
                        },
                    }
                );

                if (!userInfoResponse.ok) {
                    throw new Error('Failed to get user info from Google');
                }

                // For better security, we should use the ID token flow
                // But since we're using useGoogleLogin, we'll send the access token
                // and verify on the backend
                const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
                const response = await fetch(`${apiUrl}/api/v1/auth/google`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({
                        accessToken: tokenResponse.access_token,
                    }),
                    credentials: 'include',
                });

                const data = await response.json();

                if (!response.ok) {
                    throw new Error(data.message || 'Authentication failed');
                }

                // Store auth state
                setUser(data.user);
                setAccessToken(data.tokens.accessToken);

                if (data.tokens.refreshToken) {
                    localStorage.setItem('refreshToken', data.tokens.refreshToken);
                }

                if (data.isFirstTime) {
                    toast.success('Account created! Welcome to Free2Talk!');
                    router.push('/profile?setup=true');
                } else {
                    toast.success('Welcome back!');
                    router.push('/');
                }

                onSuccess?.();
            } catch (error) {
                console.error('Google auth error:', error);
                const message = error instanceof Error ? error.message : 'Sign-in failed';
                toast.error(message);
                onError?.(message);
            } finally {
                setIsLoading(false);
            }
        },
        onError: (error) => {
            console.error('Google login error:', error);
            toast.error('Google sign-in failed');
            onError?.('Google sign-in failed');
        },
        flow: 'implicit',
    });

    const buttonText = mode === 'signup' ? 'Sign up with Google' : 'Sign in with Google';

    return (
        <button
            type="button"
            onClick={() => googleLogin()}
            disabled={isLoading}
            className={cn(
                'w-full flex items-center justify-center gap-3 px-4 py-3',
                'bg-white hover:bg-gray-50 active:bg-gray-100',
                'text-gray-700 font-medium text-sm',
                'border border-gray-300 rounded-xl',
                'transition-all duration-200',
                'disabled:opacity-50 disabled:cursor-not-allowed',
                'focus:outline-none focus:ring-2 focus:ring-primary-500/50',
                className
            )}
        >
            {isLoading ? (
                <Loader2 className="h-5 w-5 animate-spin" />
            ) : (
                <GoogleIcon />
            )}
            <span>{isLoading ? 'Signing in...' : buttonText}</span>
        </button>
    );
}

/**
 * Google Sign-In Button Component
 * Wrapper that only renders when Google OAuth is configured
 */
export function GoogleSignInButton(props: GoogleSignInButtonProps) {
    // Don't render if Google OAuth is not configured
    if (!isGoogleOAuthConfigured()) {
        return null;
    }

    return <GoogleSignInButtonInner {...props} />;
}
